...
export default function Dashboard() {
  ...
  return (
    <div className="w-full px-6 py-4">
      <h1 className="text-3xl font-bold mb-6 text-blue-800 dark:text-blue-300 tracking-tight">Dashboard Monitoring</h1>
      
      {/* SENSOR CARDS */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">

        <Card title="Temperature" value={`${latest.temperature?.toFixed(1) || 0}°C`}
          bg="bg-orange-200 dark:bg-orange-900/40"
          text="text-orange-800 dark:text-orange-200"
        />

        <Card title="pH" value={`${latest.ph?.toFixed(1) || 0}`}
          bg="bg-sky-200 dark:bg-sky-900/40"
          text="text-sky-800 dark:text-sky-200"
        />

        <Card title="TDS" value={`${latest.tds || 0} PPM`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <Card title="DO Virtual" value={`${latest.do_virtual?.toFixed(1) || 0} mg/L`}
          bg="bg-purple-200 dark:bg-purple-900/40"
          text="text-purple-800 dark:text-purple-200"
        />

        <Card title="DO Real-time" value={`${latest.do_real?.toFixed(1) || 0} mg/L`}
      ...
      <div className="rounded-2xl shadow-xl overflow-hidden
        ...
        <div className="p-5 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-xl font-bold mb-2 text-blue-800 dark:text-blue-300 tracking-tight"> Riwayat Data Sensor</h3>
        </div>
            ...
            <tbody>
              {history.map((d, i) => (
                <tr key={i} className="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                  ...
                  <Td>{d.tds} PPM</Td>
                  <Td>{d.do_real?.toFixed(1)} mg/L</Td>
                  <Td>{d.do_virtual?.toFixed(1)} mg/L</Td>
                  <Td>
